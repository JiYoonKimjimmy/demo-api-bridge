...
test_service_running() {
    local port=$1
    if curl -s -f "http://localhost:$port/abs/health" > /dev/null 2>&1; then
        return 0
    else
        return 1
    fi
...
send_graceful_shutdown() {
    local port=$1
    print_info "Sending graceful shutdown request to http://localhost:$port/abs/shutdown..."

    local response
    local http_code

    response=$(curl -s -w "%{http_code}" -X POST "http://localhost:$port/abs/shutdown" -o /tmp/shutdown_response.json)
